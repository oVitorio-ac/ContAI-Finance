...
# S3 Bucket para arquivos CSV
resource "aws_s3_bucket" "csv_storage" {
  bucket = "${var.project_name}-csv-storage-${random_string.suffix.result}"
}
...
# ECR Repository para a aplicação
    ...
    Statement = [
      {
        ...
        Resource = "${aws_s3_bucket.csv_storage.arn}/*"
      }
    ]
  })
}
...
# Lambda para processamento de CSV
resource "aws_lambda_function" "csv_processor" {
  ...
  environment {
    variables = {
      S3_BUCKET = aws_s3_bucket.csv_storage.bucket
    }
  }
}
...
data "archive_file" "lambda_zip" {
  type        = "zip"
  output_path = "csv_processor.zip"
  source {
    content = templatefile("${path.module}/lambda_function.py", {
      bucket_name = aws_s3_bucket.csv_storage.bucket
    })
